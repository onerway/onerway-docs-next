...
[Set up a webhook event handler](mock/webhooks#set-up-webhook-handler) so Onerway can send payment events directly to your server, bypassing the client entirely. If webhook notification [delivery fails](/mock/webhooks){tooltip="Merchant failed to respond correctly"}, Onerway will [retry multiple times](mock/webhooks#webhook-retry).
...
::docs-see-also
  ...
  ::docs-see-also-item{to="https://docs.onerway.com/apis/" title="API Documentation" description="Complete API reference documentation"}
  ::

  ::docs-see-also-item{to="/get-started/start-building/start-developing"}
  ::
